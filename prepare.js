...
// It is then picked up by `wasm-opt`, which optimizes it and generates `./src/smoldot/autogen/tmp.wasm`.
...
try {
    if (build_profile == 'release') {
        child_process.execSync(
            "wasm-opt -o src/smoldot/autogen/tmp.wasm -Os --strip-debug --vacuum --dce "
            + "./backend-rust/target/wasm32-wasi/" + build_profile + "/polkadot_events_js.wasm",
            { 'stdio': 'inherit' }
        );
    } else {
        fallback_copy = true;
...
if (fallback_copy) {
    fs.copyFileSync("./backend-rust/target/wasm32-wasi/" + build_profile + "/polkadot_events_js.wasm", "./src/smoldot/autogen/tmp.wasm");
}

// We then base64-encode the `.wasm` file, and put this base64 string as a constant in
// `./src/smoldot/autogen/wasm.js`. It will be decoded at runtime.
let wasm_opt_out = fs.readFileSync('./src/smoldot/autogen/tmp.wasm');
let base64_data = wasm_opt_out.toString('base64');
fs.writeFileSync('./src/smoldot/autogen/wasm.js', 'export default "' + base64_data + '";');
fs.unlinkSync("./src/smoldot/autogen/tmp.wasm");
